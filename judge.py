...
Uses cloud LLM (DeepSeek) for semantic-level capability illusion detection.
API key: reads from env DEEPSEEK_API_KEY, or from ~/.openclaw/openclaw.json.
User must configure API key before use.
"""
import json, sys, os, re

# ── Config ──────────────────────────────────────────────────────────
def get_api_config():
    """Get DeepSeek API config. Priority: env > openclaw.json."""
    ...
    if not api_key:
        ...
        try:
            with open(config_path) as f:
                raw = f.read()
            m = re.search(r'"deepseek"\s*:\s*\{[^}]*?"apiKey"\s*:\s*"([^"]+)"', raw)
            if m:
                api_key = m.group(1)
            m2 = re.search(r'"deepseek"\s*:\s*\{[^}]*?"baseUrl"\s*:\s*"([^"]+)"', raw)
            if m2:
                base_url = m2.group(1)
        except Exception:
            pass
...
def call_deepseek(reply_text: str, user_context: str = "") -> dict:
    ...
    if not api_key:
        return {"risk": "ERROR", "reason": "No API key. Set DEEPSEEK_API_KEY env or configure ~/.openclaw/openclaw.json"}
