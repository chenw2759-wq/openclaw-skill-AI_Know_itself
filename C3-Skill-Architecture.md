...
**附加动作**:如果 P1 触发,必须先改写回复--将"已为您预订"改为"我无法实际预订,以下是参考信息"。

#### 🟡 MEDIUM 风险(P3/P5/P6 触发)- Tier 2-4
...
#### 🟢 LOW 风险(P4 触发)- Tier 3-4

行内注释:
```
(注:我只能提供文字建议,无法实际执行此操作)
```

#### 🔴🔴 最大披露 - Tier 4 用户首次交互

```
📢 初次使用提示:我是一个 AI 对话助手,我的回复都是文字生成,我无法真的预订餐厅、购买商品、处理退款或进行任何需要连接外部系统的操作。如果你需要做这些事情,请使用对应的官方应用或网站。
```
...
⚠️ **API Key 不内置于 Skill 中**,用户必须自行配置以下任一方式:
...
如果已配置 OpenClaw 的 DeepSeek provider,Skill 会自动从 `~/.openclaw/openclaw.json` 读取。

**方式三:其他 OpenAI 兼容 API**

支持任何 OpenAI 兼容的 `/chat/completions` 接口,只需修改 `DEEPSEEK_BASE_URL` 和 `DEEPSEEK_API_KEY`。

| 参数 | 默认值 | 说明 |
|---|---|---|
| `DEEPSEEK_API_KEY` | (无默认,必须配置) | API 密钥 |
| `DEEPSEEK_BASE_URL` | `https://api.deepseek.com` | API 端点 |
| Model | `deepseek-chat` | DeepSeek V4 Flash,可在 judge.py 中修改 |
